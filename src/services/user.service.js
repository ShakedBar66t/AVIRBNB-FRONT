...
export const userService = {
    ...
    logout, ///back
    signup, ///back
    getLoggedinUser, ///session storage
    saveLocalUser,
    getUsers, ///back
    getById, ///back
    remove, ///back - never used 
    update, /// back - never used 
    changeScore, //// never used
    save //
}
...
async function login(credentials) {
    console.log(credentials, 'from service')
    try {
        const loggedinUser = await httpService.post(BASE_URL + 'login/', credentials)
...
function _setLoggedinUser(user) {
    const userToSave = { _id: user._id, fullname: user.fullname, imgUrl: user.imgUrl, email: user.email }
...
async function save(user) {
    var savedUser
    if (user._id) {
        let currUser = await getById(user._id)
        user.password = currUser.password
        savedUser = await httpService.put(`user/${user._id}`, user)
    } else {
        user = userService.getLoggedinUser()
...
async function update({ _id, score }) {  /// never used
    const user = await storageService.get(USER_STORAGE_KEY, _id)
    user.score = score
    await storageService.put(USER_STORAGE_KEY, user)
